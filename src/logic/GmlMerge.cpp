...
void GmlMerge::Merge(const std::unique_ptr<XmlConfig> &cfg, const std::vector<std::unique_ptr<GmlObject>> &baseObjects, std::unique_ptr<GmlObject> &mergedObject)
{
...
void GmlMerge::MergeOne(const std::unique_ptr<XmlConfig> &cfg, const std::unique_ptr<GmlObject> &sourceObject, std::unique_ptr<GmlObject> &mergedObject)
{
    // std::string extension = cfg.get()->get("gml_extension", "");
