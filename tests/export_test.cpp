...
class GmlExportTest: public ::testing::Test
{
protected:
    std::string testFile;
...
TEST_F(GmlExportTest, ImportFileNotFoundThrows)
{
...
// TEST_F(GmlExportTest, IdSearchTest)
// {
//     auto sourceObj = std::make_unique<GmlObject>();
//     auto destObj = std::vector<std::unique_ptr<GmlObject>>();
...
//         GmlDivide::Divide(cfg, sourceObj, std::vector<NamespacePrefix>{"ot"}, destObj);
//         std::vector<xmlNodePtr> idNodes;
//         for (auto &[gmlId, nodePtr] : destObj[0].get()->getGmlStorage().getGmlMap()["ot"]) 
//         {
//             if (nodePtr.get()->children != nullptr) 
//             {
//                 tree_operations::get_xmlNodes_with_attr(nodePtr.get()->children, idNodes, "id", "gml");
//             }
//         }
//         std::cout<<idNodes.size()<<'\n';
//     });
// }